...
class WikiSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        for url in urls:
            yield scrapy.Request(url=urllib.parse.urljoin(self.base_url, url), callback=self.parse_artist)
    ...
    def spider_closed(self):
        ...
        if self.is_persistent:
            try:
                self.logger.info(f'#Crawled {self.state["artist_count"]} artists and {self.state["band_count"]} bands')
            except:
    ...
    def _clean_list(self, l):
        ...
        clean_l = [item.translate(str.maketrans('', '', string.punctuation)).strip() for item in l]
    ...
    def parse_band(self, response):
        ...
        if response_status != 200:
            self.logger.warning(f'({response_status}) for {page_url}, putting back in queue')
            yield scrapy.Request(url=urllib.parse.urljoin(self.base_url, page_url), callback=self.parse_band)
            return
        ...
        if self.is_persistent:
            ...
            self.logger.info(f'({response_status}) [b={len(band_urls)}, a={len(artist_urls)}] crawled band #{self.state["band_count"]} {page_url}')
        else:
            self.logger.info(f'({response_status}) [b={len(band_urls)}, a={len(artist_urls)}] crawled band {page_url}')
    ...
    def parse_artist(self, response):
        ...
        if response_status != 200:
            self.logger.warning(f'({response_status}) for {page_url}, putting back in queue')
            yield scrapy.Request(url=urllib.parse.urljoin(self.base_url, page_url), callback=self.parse_artist)
            return
        ...
        if self.is_persistent:
            ...
            self.logger.info(f'({response_status}) [b={len(band_urls)}] crawled artist #{self.state["artist_count"]} {page_url}')
        else:
            self.logger.info(f'({response_status}) [b={len(band_urls)}] crawled artist {page_url}')
        ...
        if len(band_urls) > 0:
            for url in band_urls:
                if url is not None:
                    yield scrapy.Request(url=urllib.parse.urljoin(self.base_url, url), callback=self.parse_band)
